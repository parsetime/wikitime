...
def goodevening():
	...
	print ()
	print ("pfm_07: versione del 28.05.2017")
...
#totali = datetime.datetime.strptime("00:00:00", "%H:%M %d %b %Y")
...
# -------------------------------------------------
	...
	print ("Studenti totali: ", j)
	print ()
	
	# resetta la pagina dei totali per prepararsi nel loop a scriverla
	#apre la pagina dei risultati totali e scrive l'intestazione
	fo=open('risultati_totali.csv','w') 
	fo.write("Nick Name" + "," + "Giorni"  + "," + "Ore" + "," + "Minuti"  +"," + "Ore Totali [dec]" +"," + "Ore Totali [ses]" + "\n")
	fo.close
		...
		if len(riga)>0:
			...
			try:
				...
				for line in fpage.readlines() :
					...
					i=i+1
				partenza=datetime.datetime.strptime("1900-01-01 00:00:00", "%Y-%m-%d %H:%M:%S")
				
				differenza=totali-partenza
				
				
				myHH=differenza.seconds//3600
				mymm=(differenza.seconds//60)%60
				myDD=differenza.days
				
				mytotalhours=myDD*24+myHH + int((mymm/60)*100)/100
				
				#print (myDD, " - ", myHH, " - ", mymm, " - ", mytotalhours)
				
				applicazione=str(totali)
				
				mydiff=str(differenza)
				
				mytotalsess=myDD*24+myHH
				
				myhourmin=str(mytotalsess)+":"+str(mymm)
				...
				fo.write(studente + "," + str(myDD) + "," + str(myHH) + "," + str(mymm) +"," + str(mytotalhours) + "," + myhourmin+ "\n")
				
				fo.close							# chiude momentaneamente il file dei totali
			
				fpage.close()						# chiude la pagina dello studente
				print ("Analizzata pagina sudente: ", studente, " -  Ore totali di lavoto: ", mytotalhours)
				os.remove(studente + ".txt")
			except:
				#pass
				print ("Errore pagina studente: " + studente)
